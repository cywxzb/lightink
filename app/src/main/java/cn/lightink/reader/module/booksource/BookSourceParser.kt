...
class BookSourceParser(val bookSource: BookSource) {
    ...
    private fun <T> getCache(key: String): T? {
        val (timestamp, value) = parseCache[key] ?: return null
    ...
    private fun saveCache(key: String, value: Any) {
        ...
        if (parseCache.size >= MAX_CACHE_SIZE) {
            //移除最旧的缓存
            val oldestKey = parseCache.entries.minByOrNull { it.value.first }?.key
            oldestKey?.let { parseCache.remove(it) }
        }
        parseCache[key] = Pair(System.currentTimeMillis(), value)
    }
    
    /**
     * 生成缓存键
     */
    private fun generateCacheKey(type: String, vararg params: String): String {
        return "${bookSource.url}_${type}_${params.joinToString("_")}"
    }
     ...
     * 执行搜索（提取的共同搜索逻辑）
     */
    private fun performSearch(key: String): List<SearchMetadata> {
        ...
        return if (results.isNullOrEmpty()) {
            ...
            if (metadata.name.contains(key, true) || metadata.author.contains(key, true)) listOf(metadata.toSearchMetadata()) else emptyList()
        } else {
            results.filter { it.name.contains(key, true) || it.author.contains(key, true) }
        }
    }
    ...
    fun findDetail(metadata: SearchMetadata): DetailMetadata? {
        //检查缓存
        val cacheKey = generateCacheKey("detail", metadata.detail)
    ...
    fun findCatalog(metadata: DetailMetadata, chapters: MutableList<BookSourceResponse> = mutableListOf(), urls: MutableList<String> = mutableListOf()): List<Chapter> {
        //检查缓存（只缓存完整目录）
        if (chapters.isEmpty() && urls.isEmpty()) {
            val cacheKey = generateCacheKey("catalog", metadata.catalog.toString())
            getCache<List<Chapter>>(cacheKey)?.let { return it }
        }
...
//        val metadata = findDetailMetadata(book.link, response)
    ...
    private fun findValueByType(response: BookSourceResponse, query: Query, isHref: Boolean, valueProvider: (String, Any, Uri) -> String): String {
        val variables = Regex("""(?<=\$\{).+?(?=\})""").findAll(query.query).map { it.value }.toSet()
        val uri = Uri.parse(response.url)
        return if (variables.isEmpty()) {
            //不存在查询模板
            val value = valueProvider(query.query, response.body, uri)
            findValueByQuery(value, response.url, query, isHref)
        } else {
            //存在至少1个查询模板
            var value = query.query
            variables.forEach { variable ->
                value = value.replace("\${$variable}", valueProvider(variable, response.body, uri))
            }
            findValueByQuery(value, response.url, query, isHref)
        }
    }
    ...
    private fun findValueByJson(response: BookSourceResponse, query: Query, isHref: Boolean): String {
        return findValueByType(response, query, isHref) { variable, body, uri ->
            findJsonValueByVariable(variable, body as String, uri)
        }
    }
    ...
    private fun findValueByHtml(response: BookSourceResponse, query: Query, isHref: Boolean): String {
        return findValueByType(response, query, isHref) { variable, body, uri ->
            findHtmlValueByVariable(variable, body, isHref, uri)
        }
    }
