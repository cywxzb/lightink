...
import cn.lightink.reader.databinding.ActivityReaderBinding
import cn.lightink.reader.databinding.ItemPageArticleBinding
import cn.lightink.reader.databinding.ItemPageAuthBinding
import cn.lightink.reader.databinding.ItemPageEndBinding
import cn.lightink.reader.databinding.ItemPageErrorBinding
import cn.lightink.reader.databinding.ItemPageIllustrationBinding
import cn.lightink.reader.databinding.ItemPageLoadingBinding
...
class ReaderActivity : LifecycleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        behavior = BottomSheetBehavior.from(binding.mReaderMenuLayout)
...
//        controller.autoSynchronize().observe(this) { showProgressDialog(it) }
        ...
        binding.mReaderPager.post {
            setupDisplay(binding.mReaderPager.height)
            setupMenuView()
    ...
    private fun setupView(savedInstanceState: Bundle?) {
        binding.mReaderLoadingTitle.typeface = FontModule.mCurrentFont.typeface
        binding.mReaderLoadingTitle.text = controller.book.chapterName
        binding.mReaderLoading.isVisible = savedInstanceState == null
        binding.mReaderLoading.setOnClickListener { showOrHideMenu(View.VISIBLE) }
        //下拉监听
        binding.mReaderFlexLayout.setOnRefreshListener {
            addOrRemoveBookmark()
            binding.mReaderFlexLayout.finishRefresh(0)
        }
        binding.mReaderFlexLayout.post { binding.mReaderFlexLayout.setDragRate(binding.mReaderBookmarkHeader.height * 2F / binding.mReaderFlexLayout.height) }
        //阅读器
        binding.mReaderPager.clearAnimator()
        binding.mReaderPager.layoutManager = layoutManager
        binding.mReaderPager.adapter = adapter
        binding.mReaderPager.addOnScrollListener(onPageChangeCallback())
        ...
        if (layoutManager.orientation == LinearLayoutManager.HORIZONTAL) {
            binding.mReaderPager.setPadding(0)
            pagerHelper.attachToRecyclerView(binding.mReaderPager)
        } else {
            binding.mReaderPager.setPadding(0, controller.display.top, 0, 0)
            pagerHelper.attachToRecyclerView(null)
        }
        controller.pullBookmarkEnableLiveData.observe(this, Observer { binding.mReaderFlexLayout.setEnableRefresh(it) })
    ...
    private fun setupTheme() {
        ...
        when {
            Preferences.get(Preferences.Key.MIPMAP_FOLLOW, false) -> {
                binding.mReaderFlexLayout.parentView.setBackgroundColor(controller.theme.background)
                if (controller.theme.mipmap.isNotBlank()) {
                    binding.mReaderLoading.background = UIModule.getMipmapByTheme(controller.theme)
                } else {
                    binding.mReaderLoading.setBackgroundColor(controller.theme.background)
                }
            }
            controller.theme.mipmap.isNotBlank() -> {
                binding.mReaderFlexLayout.parentView.background = UIModule.getMipmapByTheme(controller.theme)
                binding.mReaderLoading.background = UIModule.getMipmapByTheme(controller.theme)
            }
            else -> {
                binding.mReaderFlexLayout.parentView.setBackgroundColor(controller.theme.background)
        ...
        binding.mReaderFlexLayout.parentView.setPadding(0, if (hasVerticalAndStatusBar) statusBarHeight else 0, 0, 0)
        //Loading
        (binding.mReaderLoading.getChildAt(0) as ProgressBar).indeterminateTintList = ColorStateList.valueOf(controller.theme.control)
        binding.mReaderLoadingTitle.typeface = controller.paint.typeface
        binding.mReaderLoadingTitle.setTextColor(controller.theme.content)
        //书签
        binding.mReaderBookmarkHeader.setMargin(controller.display.horizontal)
        binding.mReaderBookmarkHeader.setTint(controller.theme.foreground, controller.theme.control)
        setupMenuTheme()
    }
    ...
    private val initializer = Observer<List<Page>?> { list ->
        ...
        if (list.isNullOrEmpty()) {
            ...
            binding.mReaderLoading.isVisible = true
            binding.mReaderLoadingTitle.text = controller.catalog.getOrNull(controller.book.chapter)?.title
            return@Observer
        }
        //读取完成
    ...
    private fun scrollToPosition(position: Int) {
        ...
        if (layoutManager.orientation == LinearLayoutManager.HORIZONTAL) {
            ...
            binding.mReaderPager.postDelayed({
