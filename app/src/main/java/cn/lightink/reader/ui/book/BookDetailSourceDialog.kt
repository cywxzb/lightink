...
import cn.lightink.reader.databinding.DialogBookDetailSourceBinding
import cn.lightink.reader.databinding.ItemBookDetailSourceBinding
...
class BookDetailSourceDialog(context: Context, list: List<SearchResult>, val callback: (Int) -> Unit) : BottomSheetDialog(context, R.style.AppTheme_BottomSheet) {
    ...
    private lateinit var binding: DialogBookDetailSourceBinding

    init {
        binding = DialogBookDetailSourceBinding.inflate(layoutInflater)
        setContentView(binding.root)
        binding.mRecycler.adapter = buildAdapter().apply { submitList(list) }
        binding.mCancel.setOnClickListener { dismiss() }
    }

    private fun buildAdapter() = ListAdapter<SearchResult>(R.layout.item_book_detail_source) { item, result ->
        val binding = ItemBookDetailSourceBinding.bind(item.view)
        binding.mBookDetailSourceName.text = result.source.name
        binding.mBookDetailSourceUrl.text = when {
        ...
        binding.mBookDetailSourceName.setDrawableStart(when {
