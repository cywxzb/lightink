...
import cn.lightink.reader.databinding.ActivityBookDetailBinding
...
class BookDetailActivity : LifecycleActivity() {
    ...
    private fun setupWithSearch() {
        ...
        binding.mBookDetailName.text = book!!.name
        binding.mBookDetailAuthor.text = book!!.author
        binding.mBookDetailCover.hint(book!!.name).load(book!!.cover)
    ...
    private fun setupView() {
        //标题显示
        binding.mBookDetailAppBar.addOnOffsetChangedListener(AppBarLayout.OnOffsetChangedListener { appBarLayout, verticalOffset ->
            binding.mTopbar.text = if (appBarLayout.getChildAt(0).height < -verticalOffset) binding.mBookDetailName.text.toString() else EMPTY
        })
        //显示简介
        binding.mBookDetailSummary.setOnClickListener {
            if (binding.mBookDetailSummary.text.isBlank()) return@setOnClickListener
            WarningMessageDialog(this, binding.mBookDetailSummary.text.toString()).show()
        }
        //设置目录排序菜单
        binding.mBookDetailCatalogLayout.inflateMenu(R.menu.sort)
        binding.mBookDetailCatalogLayout.setOnMenuItemClickListener { item ->
            Preferences.put(Preferences.Key.BOOK_DETAIL_SORT, item.itemId == R.id.menu_sort_ascending)
            setupMenu()
        }
        //设置RV
        binding.mBookDetailRecycler.layoutManager = RVLinearLayoutManager(this)
        binding.mBookDetailRecycler.adapter = adapter
        //书源列表
        binding.mBookDetailSource.setOnClickListener { showBookSourceDialog() }
        //设置目录排序
        setupMenu()
    }
    ...
    private fun setupMenu(): Boolean {
        ...
        binding.mBookDetailCatalogLayout.menu.findItem(R.id.menu_sort_ascending).isVisible = !isDescending
        binding.mBookDetailCatalogLayout.menu.findItem(R.id.menu_sort_descending).isVisible = isDescending
        val layoutManager = binding.mBookDetailRecycler.layoutManager as LinearLayoutManager
    ...
    private fun queryBookDetail(position: Int = 0): LiveData<DetailMetadata> {
        ...
        binding.mBookDetailCatalogLayout.visibility = View.INVISIBLE
        binding.mBookDetailRead.visibility = View.INVISIBLE
        binding.mBookDetailLoading.isVisible = true
        binding.mBookDetailSource.isEnabled = book?.list?.get(index)?.source?.name.isNullOrBlank().not()
        binding.mBookDetailSource.text = book?.list?.get(index)?.source?.name
    ...
    private fun setupBookMetadata(metadata: DetailMetadata) {
        binding.mTopbar.setMenuVisible(URLUtil.isNetworkUrl(metadata.url))
        binding.mTopbar.setOnMenuClickListener { openOriginWeb(metadata.url) }
        //封面地址
        if (!URLUtil.isNetworkUrl(book?.cover) && URLUtil.isNetworkUrl(metadata.cover)) {
            book?.cover = metadata.cover
            binding.mBookDetailCover.hint(book?.name.orEmpty()).load(book?.cover)
        }
        //作者
        ...
        //作者快速搜索
        binding.mBookDetailAuthor.setOnClickListener {
            startActivity(Intent(this, SearchActivity::class.java).putExtra(INTENT_SEARCH, book!!.author))
        }
        binding.mBookDetailSummary.text = book?.summary
        binding.mBookDetailCatalogTitle.text = when {
        ...
        if (book != null && Room.book().has(book!!.objectId())) {
            ...
            binding.mBookDetailLoading.isVisible = false
            binding.mBookDetailRead.isEnabled = true
            binding.mBookDetailRead.isVisible = true
            binding.mBookDetailRead.setOnClickListener { openBook(Room.book().get(book!!.objectId())) }
        }
    }
    ...
    private fun setupBookResult(chapters: List<Chapter>?) {
        ...
        binding.mBookDetailCatalogLayout.isVisible = true
        binding.mBookDetailCatalogLayout.menu.findItem(R.id.menu_sort_descending).isEnabled = true
        binding.mBookDetailLoading.isVisible = false
        //书架不存在同名图书
        if (!binding.mBookDetailRead.isVisible) {
            binding.mBookDetailRead.isVisible = true
            binding.mBookDetailRead.isEnabled = chapters?.isNotEmpty() == true
            binding.mBookDetailRead.text = getString(if (binding.mBookDetailRead.isEnabled) R.string.book_detail_insert_bookshelf else R.string.book_detail_loading_failed)
            binding.mBookDetailRead.setOnClickListener { getPreferredBookshelf() }
        }
    }
    ...
    private fun publish(bookshelf: Bookshelf) {
        ...
        controller.publish(book, bookshelf).observe(this, Observer { book ->
            ...
            if (book != null) {
                intent.putExtra(INTENT_BOOK, book.objectId)
                binding.mBookDetailRead.text = getString(R.string.book_detail_immediately_read)
                binding.mBookDetailRead.setOnClickListener { openBook(book) }
            } else {
                binding.mBookDetailRead.text = getString(R.string.book_detail_insert_bookshelf)
            }
        })
    }
    ...
    private fun preview(index: Int, chapter: Chapter) {
        if (binding.mBookDetailLoading.isVisible) return toast("正在读取目录，稍候再试")
        //已加入的书架不能预览
    ...
    private fun buildAdapter() = ListAdapter<Chapter>(R.layout.item_book_detail_chapter) { item, chapter ->
        ...
        item.view.setOnClickListener { preview(binding.mBookDetailRecycler.adapter!!.itemCount - 1 - item.adapterPosition, chapter) }
    }
}
